...
export async function PrintDoc({
  ...
  return (
    <div className="print-root">
      ...
      <div
        className="print-doc mx-auto rounded-lg bg-white p-8 shadow-xl"
        style={{ maxWidth: landscape ? "1100px" : "820px" }}
      >
        {/* Letterhead */}
        <div className="flex items-start justify-between gap-6 border-b-2 border-gray-800 pb-3">
          <div>
            ...
            {seller.addressLines.map((l) => (
              <p key={l} className="text-xs text-gray-600">{l}</p>
            ))}
          ...
          <div className="text-right text-xs text-gray-600">
            {tax.taxId && <p>BIN / VAT reg.: <span className="font-semibold text-gray-900">{tax.taxId}</span></p>}
        ...
        {signatures.length > 0 && (
          <div className="mt-16 grid gap-8" style={{ gridTemplateColumns: `repeat(${signatures.length}, 1fr)` }}>
            {signatures.map((s) => (
              <div key={s} className="border-t border-gray-700 pt-1 text-center text-xs text-gray-700">{s}</div>
            ))}
          </div>
        )}

        <p className="mt-8 border-t border-gray-200 pt-2 text-center text-[10px] text-gray-400">
